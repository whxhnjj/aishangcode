        ...
        if (url == 'invite'){
          if (data.detect) {
            if(that.globalData.member){
              if (that.globalData.member.mobile){
                wx.showModal({
                  title: '团队邀请',
                ...
                wx.showModal({
                  title: '提示',
                  content: res.data.creator.realname ? res.data.creator.realname : res.data.creator.nickname + '邀请你加入“' + res.data.creator.team_name + '”团队，请完善个信息后重新点击小程序名片',
                });
              }
            }else{
              that.memberCallback = function(member){
  ...
  getMemberInfo: function () {
    ...
    core.request({
      ...
      success: function (res) {
        ...
        that.memberCallback && that.memberCallback(res.data.member);
      },
      fail: function (res) {
      }
    });
