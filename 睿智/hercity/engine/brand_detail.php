...
$brands = getArrayFromBrands(" where id=".$id);
$brand = $brands[0];
...
$brand_news = getArrayFromPosts("where is_show and dateline<=UNIX_TIMESTAMP() and keyword like '%@".$brand['firstname']."%' and (kind_id in (".kindsGetChildren(6).") or kind_id in (".kindsGetChildren(8).") or kind_id in (".kindsGetChildren(1).") or kind_id in (".kindsGetChildren(2).") or kind_id=102) order by order_id desc,id desc limit 5",22);
$brand_photo = getArrayFromPosts("where is_show and dateline<=UNIX_TIMESTAMP() and keyword like '%@".$brand['firstname']."%' and (kind_id in (".kindsGetChildren(7).") or kind_id=103) order by order_id desc,id desc limit 15",34);
