...
if ($tag == ''){$tag = 'tag';}
if ($tag != 'tag'){$where .= " and (tags like '%".$tag."%' or firstname like '%".$tag."%' or lastname like '%".$tag."%') ";}
...
$page_max_no = $recordcount - ($pagesize * ($page - 1));
	...
	//栏目文章列表
	$list = getArrayFromBrands($where." order by is_auth desc,order_id desc,id desc limit ".($page-1)*$pagesize.",".$pagesize,'id,firstname,lastname,domain,logo,tags,hits,is_auth');

	//调用分页函数
	$pages = pages($recordcount,$pagesize,$page,$config['basePath']."list-".$tag."-".$auth."-".$allow."-%d.html");
