...
list($is_list,$folder,$pagesize,$name) = $mysqli->query("select is_list,folder,pagesize,name from ".$config['tablePre']."kinds where id=".$kind_id)->fetch_row();
...
if ($is_list)
{
	...
	{
		echo "全部处理完成!<a href=\"posts_index.php?kind_id=$kind_id\">返回</a>";
	}
}
else
{
	echo "该栏目未开通生成页面功能。<a href=\"posts_index.php?kind_id=$kind_id\">返回</a>";
}
